 ...
 * @api private
 */
Package.prototype.authenticateCall = function(req) {
  var netrc = this.netrc[this.remote.host];

  if (netrc && netrc.password === 'x-oauth-basic') return req.set('authorization', 'Bearer '+netrc.login);
  if (netrc) return req.auth(netrc.login, netrc.password)
  if (this.auth) return req.auth(this.auth.user, this.auth.pass);
};
